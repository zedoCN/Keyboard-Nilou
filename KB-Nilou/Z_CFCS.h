...
int Z_CFCS_Add(String Name, unsigned long Time, Function_pointer Function)
{
    for (int a_CID = 0; a_CID < CFCS_N; a_CID++)
    {
        if (CFCS[a_CID].C_Name == "" && CFCS[a_CID].isStop == true)
        {
...
void Z_CFCS_LOOP(int D_CID)
{
    for (int a_CID = 0; a_CID < CFCS_N; a_CID++)
    {
        ...
        if (CFCS[a_CID].C_Name != "") //检测CFCS是否可用
        {
            ...
            {
                if (CFCS[a_CID].C_CFCS_D == true && D_CID != -1)
                {
                    continue;
                }
