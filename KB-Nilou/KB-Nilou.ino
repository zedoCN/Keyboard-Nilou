...
String str_z(String str, String left, String right)
{
    ...
    if (str.substring(lw - left.length(), lw) != left || rw == 0)
    {              //判断欲寻找的内容是否存在
        return ""; //没找到 返回空
    }
...
int CLOCK_Key_Light(int CID)
{
    ...
    FastLED.setBrightness(FastLED.getBrightness() + ((lightBrightness * 17) - FastLED.getBrightness()) * lightTransition);
...
int CLOCK_Serial(int CID) {
    if (Serial.available() > 0) {
        char cmd = Z_Serial_Read_byte();//读取命令
        ...
        case CMD_READ_C:
        {
            char id = Z_Serial_Read_byte();//读编号
        ...
        case CMD_PLAY:
        {
            char len = Z_Serial_Read_byte();//读长度
            char au_name[len];
...
//音频命令  命令    参数    参数大小
void send_audio_cmd(char order, char* p, int size) {
    char verify = 0;
    char len = 3 + size;
    char send_order[len + 2];
    send_order[0] = 0x7E;
...
void RE_Event_Long_press(int CID) {
    ...
    case 1://1模式选择
        ...
        case 2:
            ...
            digitalWrite(MODE_SWTCH, HIGH);//音频编辑
            ...
            shake(true);
            Z_CFCS_Delay(200, CID);
            shake(true);
            digitalWrite(MODE_SWTCH, LOW);//音频编辑
            Z_CFCS_Delay(1000, CID);
